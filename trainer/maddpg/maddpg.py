...
class MADDPGAgentTrainer(AgentTrainer):
    ...
    def sync_target_nets(self):
        make_update_exp(self.p_network, self.target_p_network, rate=1.0)
        make_update_exp(self.q_network, self.target_q_network, rate=1.0)
    ...
    def update(self, agents, t):
        ...
        next_act_n = []
        for i in range(self.n):
            next_act_i = _get_action(agents[i].target_p_network, obs_n, index=i, device=self.args.device)
            if self.args.device == 'cpu':
                next_act_i = torch.tensor(next_act_i.clone().detach().numpy()).to(self.args.device)
            else:
                next_act_i = torch.tensor(next_act_i.cpu().clone().detach().numpy()).to(self.args.device)
            next_act_n.append(next_act_i)
        

        # Compute q_loss
        q_loss, q_value, target_q_value = _q_train(self.q_network, self.target_q_network, obs_n, act_n, reward, obs_next_n, next_act_n, self.agent_index, gamma=self.args.gamma, done=done, device=self.args.device)
